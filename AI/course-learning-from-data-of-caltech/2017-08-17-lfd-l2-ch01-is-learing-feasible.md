...
$$\boldsymbol P[\lvert \nu - \mu \rvert > \epsilon] \leq 2e^{-2 \epsilon^2 N}$$

$$\lvert \nu - \mu \rvert > \epsilon$$ 发生的概率虽然与其中三者都有关, 但是其边界依赖于 $$\epsilon$$ 和样本数量 N. 我们只知道 $$\nu$$ 依赖于我们的 N 个随机样本, $$\mu$$ 并不是随机的, 它是个常数, 也只有上帝知道它是多少.  
...
训练集是从样本空间 X (一定的概率分布)中得到的. N 个训练集就类似于我们随机从罐子中取得的N个样本小球.

这时候, 如果我们通过算法A训练从假设集 hypothesis set 中确定了一个假设 h, 那么这个假设 h 在训练集(x)上, $$h(x) = y = f(x)$$, 则该样本小球x为绿球, 否则 $$h(x) \neq y = f(x)$$ 该样本小球x为红球. 这就会得到在红球占 N 个样本球总数的比为 $$\nu$$.
